...
(function () {
  ...
  Screens.course = (params) => {
    ...
    /* --- الدخول عند المحتوى مباشرةً -------------------------------------------
       من فتح المادة من «موادّي» اختارها وسدّد ثمنها ويريد درسه، فإجباره على
       تمرير شاشةٍ كاملة في كل مرّة ضريبةٌ يدفعها يوميًّا. الغلاف يبقى فوقه
       إن أراده — نبدأ عنده لا نحذفه.

       والقياس حيٌّ لا رقمٌ مكتوب: ارتفاع الغلاف نسبةٌ من عرض الشاشة والورقة
       تطول بطول الوصف، فأي رقمٍ يصحّ على جهازٍ ويخطئ على آخر. وبعد الإدراج
       في DOM لا قبله — قبله كل المقاسات أصفار فتصير القفزة إلى الصفر. */
    if (params.atContent) {
      /* الترويسة تُرفع **تزامنيًّا** ومستقلّةً عن القفزة: نداء المراقب غير
         متزامن، فبدونها تُفتح الشاشة بشريطٍ فارغ جزءًا من الثانية ثم يظهر
         الاسم — وميضٌ يبدو عطلًا. والمراقب يؤكّدها بعد أوّل تمرير. */
      nameBar.classList.add('is-on');
      scrim.classList.add('is-on');

      // القفزة وحدها تحتاج إطارًا: قبل الإدراج في DOM كل المقاسات أصفار
      if (typeof requestAnimationFrame === 'function') {
        requestAnimationFrame(() => {
          body.scrollTop = sentinel.getBoundingClientRect().top
