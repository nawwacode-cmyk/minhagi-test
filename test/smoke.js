...
{
  /* مقيَّدٌ بسهم بطاقة الأستاذ تحديدًا. كان يلتقط أوّل `icon.fwd` في الملفّ
     أيًّا كان، فلمّا أُضيف سهمٌ أصغر في تنبيه الاشتراك (داخل شريط ملوّن بنصّه،
     لا علامةً وحيدة في زاوية بيضاء) صار الحارس يقيس العنصر الخطأ ويسقط على
     شيفرة سليمة. */
  const call = mainSrc.match(/tcard__go',\s*icon\.fwd\((\d+), \{ width: ([\d.]+) \}\)/);
...
// --- قائمة «موادّي»: صفّ لكل مادة، وسهم يفتح أستاذها -----------------------------
// حلّت محلّ شريط الحلقات الأفقي (لم يتّسع لصورة أستاذ أصلًا). السهم يظهر بس
// لمادة لها أستاذ مطابق، ويبدّل الفتح/الطيّ بمعزلٍ عن نقرة الصفّ (تفتح المادة).
ok('القائمة صفوف .srow لا شريط .hrail القديم',
   /h\('div\.srow/.test(mainSrc) && !/hrail|\.mini\b/.test(mainSrc));
// فحوص الرسم الفعلي (أي مادة تظهر لها سهم، الطيّ الافتراضي...) بملف
// test/suites/home-list-check.js — تحتاج تشغيل الشاشة فعليًا لا نصّ المصدر
// وحده، وهذا الملف يفحص المصدر نصًّا فقط بلا تنفيذ.
// الضغط على أي مكان بالصفّ يفتح المادة — لا صفحة الأستاذ المستقلّة؛ صفّ
// الأستاذ نفسه بلا أي onclick منفصل فيرث نقرة الصفّ.
ok('نقرة الصفّ كلّه تفتح المادة', /onclick: \(\) => App\.go\('course', \{ subject: sub\.id \}\)/.test(mainSrc));
ok('صفّ الأستاذ بلا معالج نقر مستقلّ (يرث نقرة الصفّ)',
