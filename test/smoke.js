...
/* --- التقدّم: شريطٌ ورقم أسفل البطاقة ------------------------------------------
   كان صفٌّ أعلى البطاقة يحمل أيقونة المادة ونسبةً في قرص. حُذف الاثنان بطلبٍ
   صريح: البطاقة ١٥٢ بكسل عرضًا، والأيقونة تكرّر ما يقوله الاسم واللون، والقرص
   يقول رقمًا بلا مقياسٍ يُقارن به. الشريط يقول الرقم **ويُريه**.

   والشريط بعرض البطاقة كاملًا لا داخل زاوية: هذا نصّ الطلب. */
/* `\b` ضرورية: `tsubj__ico` (صفّ موادّ الأستاذ) مكوّنٌ آخر تمامًا، وتعبيرٌ
   بلا حدٍّ يلتقطه فيسقط الحارس على شيفرةٍ سليمة. */
ok('لا أيقونة مادة ولا قرص نسبة في أعلى البطاقة',
   !/\bsubj__top/.test(mainSrc) && !/\bsubj__top/.test(cssSrc)
   && !/\bsubj__ico/.test(mainSrc) && !/\bsubj__ico/.test(cssSrc));
ok('بل شريط تقدّمٍ بعرض البطاقة',
   /h\('span\.subj__prog'/.test(mainSrc)
   && /\.subj__prog \{[^}]*width: 100%/.test(cssSrc));
// والرقم يبقى بجانبه: شريطٌ وحده يقول «قليل» لا «٢٪»
ok('والنسبة رقمًا بجانبه',
   /subj__pct/.test(mainSrc) && /\.subj__pct \{/.test(cssSrc));
/* والتعبئة لها حدٌّ أدنى مرئي: عند ١٪ يصير عرضها أقلّ من بكسل فتختفي، فيبدو
   الشريط فارغًا تمامًا لمن بدأ فعلًا — وهي أكثر حالةٍ سيراها الطلاب أوّل شهر. */
ok('وتعبئةٌ تُرى ولو كانت ١٪', /\.subj__bar i \{[^}]*min-width:/.test(cssSrc));
// الاسم يبقى ملتصقًا بأسفل البطاقة بعد أن ذهب الصفّ الذي كان يدفعه
ok('والاسم ما زال مدفوعًا لأسفل البطاقة',
   /\.subj__name \{[^}]*margin-top: auto/.test(cssSrc));
...
/* --- الدخول من «موادّي» يبدأ عند المحتوى ---------------------------------------
   الغلاف تعريفٌ بالمادة وأستاذها — يخدم من يستكشف. ومن فتحها من «موادّي» قد
   اختارها وسدّد ثمنها ويريد درسه، فإجباره على تمرير شاشةٍ كاملة في كل مرّة
   ضريبةٌ يدفعها يوميًّا. يبدأ عند التبويبات، والغلاف فوقه إن أراده.

   ولا يُعمَّم على كل المداخل: الوصول من بانرٍ أو من صفحة أستاذٍ استكشافٌ،
   والغلاف هناك هو المقصود. */
ok('بطاقة «موادّي» تفتح المادة عند المحتوى',
   /onclick: \(\) => App\.go\('course', \{ subject: subject\.id, atContent: 1 \}\)/.test(mainSrc));
{
  // وبقيّة المداخل تُبقي الغلاف — التعميم يلغي المعنى
  const calls = (mainSrc.match(/App\.go\('course', \{[^}]*\}/g) || []);
  const withFlag = calls.filter((c) => /atContent/.test(c));
  ok('وهي وحدها دون بقيّة المداخل', withFlag.length === 1,
     `${withFlag.length} من ${calls.length}`);
}
/* التنفيذ بقياسٍ حيّ لا برقم: ارتفاع الغلاف نسبةٌ من عرض الشاشة والورقة
   تطول بطول الوصف — أي رقمٍ مكتوب يصحّ على جهازٍ ويخطئ على آخر. */
ok('والقفزة بقياسٍ حيّ لا برقمٍ مكتوب',
   /sentinel\.getBoundingClientRect\(\)/.test(courseSrc)
   && /body\.getBoundingClientRect\(\)/.test(courseSrc));
// وبعد الإدراج في DOM: قبله كل المقاسات أصفار فتصير القفزة إلى الصفر
ok('وبعد الإدراج في DOM لا قبله',
   /requestAnimationFrame\([\s\S]{0,120}getBoundingClientRect/.test(courseSrc));
/* وشريط الاسم يُرفع فورًا: مراقب التقاطع غير متزامن، فبدونه تُفتح الشاشة
   بترويسةٍ فارغة جزءًا من الثانية ثم يظهر الاسم — وميضٌ يبدو عطلًا. */
ok('وشريط الاسم مرفوعٌ من أوّل إطار',
   /classList\.add\('is-on'\)/.test(courseSrc));

/* --- الشدّ المطّاطي عند نهاية التمرير -------------------------------------------
   `overscroll-behavior-y: none` على `body` تمنع سحبَ الصفحة للتحديث، لكنها
   **لا تمسّ** المُمرِّر الداخلي `.screen__body`. وحين يبلغ الأخير نهايته يمطّ
   المتصفّح محتواه كلّه (الشدّ المطّاطي) — والعناصر اللاصقة تُمطّ معه فتنزلق
   الترويسة عن مكانها ثم ترتدّ.

   لم يظهر هذا قبل اليوم لأن الترويسة كانت **خارج** المُمرِّر؛ إدخالها إليه
   (كي يمرّ الغلاف مع المحتوى) هو ما عرّضها للشدّ. أُبلغ عنه صراحةً: التمرير
   سليم حتى آخر بطاقة ثم «تنكسر». */
ok('المُمرِّر الداخلي يمنع الشدّ المطّاطي',
   /\.screen__body \{[^}]*overscroll-behavior-y: none/.test(cssSrc));
// و`contain` لا تكفي: تمنع تسلسل التمرير للأب وتُبقي الارتداد المحلّي
ok('و«none» لا «contain» — الأخيرة تُبقي الارتداد',
   !/\.screen__body \{[^}]*overscroll-behavior-y: contain/.test(cssSrc));

ok('الصورة ثابتة بـsticky لا محسوبة بجافاسكربت',
   /\.chero__band \{[^}]*position: sticky/.test(cssSrc));

/* --- قياس الغلاف ---------------------------------------------------------------
   كان ٢٤٠ بكسل ثابتة — أي ٦٨٪ من عرض هاتفٍ ضيّق و٣٠٪ من لوحٍ عريض: نفس
   التصميم يبدو غلافًا بطوليًّا هنا وشريطًا رفيعًا هناك. صار **مربّعًا**
   بقياس العرض، فيحتفظ بنسبته على كل جهاز — وهي نسبة اللقطة المرجعية. */
{
  const hv = ((cssSrc.match(/\.chero__band \{[^}]*height:\s*([^;]+)/) || [])[1] || '').trim();
  const vw = Number((hv.match(/(\d+)vw/) || [])[1]);
  ok('الغلاف يقاس بعرض الشاشة لا برقمٍ ثابت', vw >= 90, hv || 'لا ارتفاع');
  // وسقفٌ بالبكسل: على حاسوبٍ عريض يصير 100vw غلافًا بطول ألف بكسل
  ok('وبسقفٍ للشاشات العريضة', /min\(/.test(hv) && /\d+px/.test(hv), hv);
  /* وسقفٌ بالارتفاع: في الوضع الأفقي يتجاوز عرضُ الشاشة ارتفاعَها، فغلافٌ
     بقياس العرض يبتلع الشاشة كلّها ولا يبقى للمحتوى شيء. */
  ok('وسقفٍ بالارتفاع للوضع الأفقي', /\d+vh/.test(hv), hv);
}
{
  ...
  /* قفزة الدخول تقرأ `scrollTop` **مرّةً واحدة** وهذا مشروع. المحظور أن
     يُقرأ خارجها: هناك يعني حسابًا يتكرّر مع التمرير، وهو العطل الذي زال. */
  const jump = (scope.match(/if \(params\.atContent[\s\S]*?\n    \}/) || [''])[0];
  ok('وقفزة الدخول معزولة في كتلتها', jump.length > 0);
  ok('ولا حساب موضعٍ من scrollTop خارجها', !/scrollTop/.test(scope.replace(jump, '')));
}
