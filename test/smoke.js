...
/* --- الترويسة المنكمشة -------------------------------------------------------
   الغلاف ينكمش **من تحت** ووجه الأستاذ يبقى ثابتًا بالأعلى — لا ينزلق لفوق
   فيبقى أسفل الصورة وحده. يتحقّق بإزاحة الصورة داخل نافذتها بمقدار ما مرّ
   من التمرير.

   وبـ`transform` قطعًا لا بـ`height`: الأولى على المُركِّب وحده، والثانية
   تعيد تخطيط الصفحة كلّها في **كل إطار** — على هاتفٍ اقتصادي ذلك تقطيعٌ
   مرئيّ في أكثر شاشةٍ يُمرَّر فيها بالتطبيق. */
ok('الغلاف ينكمش بإزاحة لا بارتفاع',
   /translateY\(\$\{fx\.shift\}px\)/.test(courseSrc)
   && !/\.style\.height\s*=/.test(courseSrc));
/* حدث التمرير يُطلق عشرات المرّات بالثانية؛ بلا خنقٍ بإطار يُحسب ما لا يُرسم.
   الفحص داخل جسم `onScroll` وحده: `requestAnimationFrame` ترد أيضًا في قياس
   الوصف، و`ticking` ترد في تصفيرها — فكلتاهما كانت تمرّ على ملفٍّ بلا خنقٍ
   إطلاقًا. (سقط هذا الحارس في مراجعة الحرّاس، فشُدّ.) */
{
  const fn = (courseSrc.match(/const onScroll = \(\) => \{[\s\S]*?\n    \};/) || [''])[0];
  ok('والاستماع مخنوق بإطارٍ واحد',
     /if \(ticking\) return;/.test(fn) && /requestAnimationFrame/.test(fn),
     fn ? 'جسم onScroll مقروء' : 'لم يُعثر على onScroll');
}
ok('وسلبيٌّ كي لا يؤخّر التمرير', /'scroll', onScroll, \{ passive: true \}/.test(courseSrc));
// ارتفاع الغلاف مكتوبٌ مرّتين (JS وCSS) — انحرافهما يترك شريطًا عالقًا
ok('وثابت الارتفاع يطابق CSS',
   /const BAND_H\s*=\s*240;/.test(courseSrc) && /\.chero__band \{[^}]*height: 240px/.test(cssSrc));

/* شريط الاسم: يظهر حين يختفي الغلاف كي لا تضيع هويّة المادة داخل القائمة.
   وبحدَّين لا حدٍّ واحد — حدٌّ واحد يعني رفرفةً مستمرّة عند أوّل بكسل حوله. */
ok('شريط الاسم مخفيٌّ حتى ينكمش الغلاف',
   /\.chero__bar \{[^}]*height: 0/.test(cssSrc) && /\.chero__bar\.is-on \{[^}]*height:/.test(cssSrc));
{
  const on  = Number((courseSrc.match(/const BAR_ON\s*=\s*(\d+)/) || [])[1]);
  const off = Number((courseSrc.match(/const BAR_OFF\s*=\s*(\d+)/) || [])[1]);
  ok('وبفجوةٍ تمنع الرفرفة عند الحدّ', on > off && off > 0, `${off} → ${on}`);
}
// في RTL يقع زرّ الرجوع العائم يمينًا — أي فوق أوّل حرفٍ من الاسم بالضبط
// القيمة لا وجود المُعلَن: `padding-inline-start: 0` يمرّ على فحص الوجود
// ولا يُفرغ شيئًا. الزرّ ٣٦ بكسل على بُعد ١٤، فالحدّ الأدنى ٥٠.
{
  const v = Number((cssSrc.match(/\.chero__bar \{[^}]*padding-inline-start:\s*(\d+)px/) || [])[1]);
  ok('واسم المادة لا يقع تحت زرّ الرجوع', v >= 44, `${v || 0}px`);
}
// ودائرةٌ بيضاء بظلّ فوق شريطٍ أبيض تبدو غلطة، فيُسطَّح الزرّ عند الانكماش
ok('والزرّ يندمج بالشريط عند الانكماش',
   /is-collapsed/.test(courseSrc) && /\.is-collapsed \.chero__back \{/.test(cssSrc));
// الانزلاق المتوازي حركةٌ صريحة تُطفأ لمن طلب تقليلها
ok('والانكماش يُطفأ لمن يطلب تقليل الحركة', /prefers-reduced-motion/.test(courseSrc));

/* الغلاف صار يمرّ، وزرّ الرجوع كان `absolute` داخله — أي يمرّ معه. نفس ما
   وقع في شاشة الأستاذ حرفيًّا: مسار الرجوع الوحيد يختفي بأوّل تمرير. */
ok('وزرّ الرجوع يبقى ثابتًا رغم مرور الغلاف',
   /\.chero__back \{[^}]*position: fixed/.test(cssSrc));
// وإخفاؤه على الحاسوب مع بقيّة الأزرار العائمة — الحارس المشترك أدناه
   ...
   إلى القائمة — على شيفرةٍ سليمة. */
{
  ...
  const floating = ['.teacher-back', '.chero__back'];
