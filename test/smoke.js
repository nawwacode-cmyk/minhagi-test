...
/* المقصود بالقرار «لا بانر في الرئيسية» هو **الشريط المتحرّك** الذي كان يشغل
   أبرز موضع بمحتوى لم يختره أحد — لا أن تُحرَم الرئيسية من أي إشارة إلى
   الأخبار. الرئيسية الآن تعرض خبرين مختصرين بزرّ «الكل» يقود إلى القسم:
   مدخلٌ إليه لا نسخةٌ منه.

   فالحارس يقيس ما قُصد فعلًا: لا دورة ولا شرائح ولا قائمة كاملة. */
{
  const home = mainSrc.slice(mainSrc.indexOf('Screens.home ='), mainSrc.indexOf('Screens.subjects ='));
  ok('لا شريط بانر متحرّك بـ«الرئيسية»',
     !/setInterval|slide|شريحة/i.test(home));
  ok('والأخبار عليها مقتطفٌ محدود لا القائمة كاملة',
     /posts\.slice\(0, ?[12]\)/.test(home));
  ok('ومعه طريقٌ إلى القسم',
     /App\.go\('news'\)/.test(home));
}
