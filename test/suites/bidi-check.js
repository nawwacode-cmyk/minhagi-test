...
// =============================================================================
// ٤) نصّ الدرس: الجدول لا يدفع الصفحة
//    درس «الكتابة» — الرابع في كل وحدة — يحمل جدول تصحيح أعرضَ من شاشة الهاتف.
//    كان يُحقن خامًا، فيفرض عرضه على عمود الشبكة وتخرج الصفحة كلّها عن الشاشة:
//    فيديو مقتطع وعنوان مبتور. `UI.prose` تلفّ كل جدول بحاويةٍ تُمرَّر وحدها.
// =============================================================================
{
  const BODY = '<h3>معايير التصحيح</h3>'
    + '<p>يوزَّع العلام على النحو التالي :</p>'
    + '<table><thead><tr><th>المعيار</th><th>النقاط</th></tr></thead><tbody>'
    + '<tr><td>احترام الشكل (البريد: De / À / objet)</td><td>٣</td></tr>'
    + '<tr><td>الملاءمة مع الوضعية</td><td>٤</td></tr>'
    + '</tbody></table>'
    + '<p>لا تتجاوز <strong>quatre-vingts</strong> كلمة.</p>';

  const out = UI.prose(BODY)._h;
  const wrapped = /<div class="prose__scroll"><table[\s\S]*?<\/table><\/div>/.test(out);
  ok('جدول الدرس ملفوفٌ بحاوية تُمرَّر', wrapped);
  ok('ولا حاوية زائدة', (out.match(/prose__scroll/g) || []).length === 1);
  // النصّ حول الجدول يبقى كما هو — اللفّ لا يقصّ ولا يعيد الترتيب
  ok('باقي نصّ الدرس سليم',
     out.replace(/<\/?div[^>]*>/g, '') === BODY);

  // جدولان في درس واحد: كلٌّ بحاويته، لا واحدة تبتلع ما بينهما
  const two = UI.prose('<table><tr><td>أ</td></tr></table><p>بين</p><table><tr><td>ب</td></tr></table>')._h;
  ok('جدولان ⇒ حاويتان', (two.match(/prose__scroll/g) || []).length === 2
     && /<\/div><p>بين<\/p><div/.test(two));

  // درسٌ بلا جدول لا يُمسّ إطلاقًا
  const plain = '<p>Bonjour ! كيف حالك؟</p>';
  ok('درس بلا جدول يمرّ كما هو', UI.prose(plain)._h === plain);
}

// =============================================================================
// ٥) نموذج الكتابة (‎<pre>‎): يلتفّ بدل أن يمتدّ خارج الشاشة
//    نفس عطل الجدول بسببٍ مختلف: ‎<pre>‎ افتراضيًا ‎white-space: pre‎ — لا يلتفّ
//    مهما كان ‎overflow-wrap‎. أربعة دروس كتابة على الإنتاج تستعمله للنموذج
//    الجاهز، وكلّها كانت تخرج عن الشاشة يمينًا ويسارًا معًا.
// =============================================================================
{
  const cssSrc = fs.readFileSync(path.join(__dirname, '..', '..', 'css', 'app.css'), 'utf8');
  // مرساة بدء السطر: نفس السلسلة الحرفية «‎.prose pre {»‎ تظهر أيضًا في نهاية
  // قائمة محدِّدات unicode-bidi (سطر ٩٩٧) — بلا هذه المرساة يلتقط أوّل ظهور خطأً.
  const rule = (cssSrc.match(/^\.prose pre \{[^}]*\}/m) || [''])[0];
  ok('‎<pre>‎ يلتفّ بدل الامتداد', /white-space:\s*pre-wrap/.test(rule), rule || 'لا قاعدة');
  ok('وينكسر بلا مواضع قطع كأي فقرة', /overflow-wrap:\s*anywhere/.test(rule), rule || 'لا قاعدة');
  ok('واتجاهه من أوّل حرف قويّ فيه كباقي ‎.prose',
     /\.prose blockquote, \.prose pre \{[^}]*unicode-bidi:\s*plaintext/.test(cssSrc));
}
